...
<template>
  <section class="animate__animated animate__fadeInRight" style="height: 100%">
    <section class="container" v-loading="loading" element-loading-background="rgba(0,0,0,0.3)">
      ...
      <remark color="#a3cccc">
        备注：‘模拟仿真数据’展示的是由探空观测数据与敏感性数据计算后得到的轨迹预测数据。
      </remark>
    </section>
  </section>
</template>
...
import { downloadpre, predata } from "../api";
...
function getTableData() {
  ...
  predata({ ...params, nowPage: paginationState.currentPage, showNum: paginationState.pageSize })
    .then((data) => {
      if (!data.success) {
...
function download(row) {
  ...
  const stationNum = row.stationNum;
  // const stationNum = getStationNum(row.station);
  if (!row.revTime) {
    ElMessage.warning("没有接收数据时间");
    return;
  }
  const date = row.revTime;
  // const date = getDate(row.revTime);
  curDownloading.value = row.station + row.revTime;

  downloadpre({ stationNum, date })
    .then((data) => {
